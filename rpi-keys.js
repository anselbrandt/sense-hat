...
class HatKeys {
  static get DEVICE_NAME() {
    return "Raspberry Pi Sense HAT Joystick";
  }
  ...
  static findInputDevice(deviceName) {
    ...
    for (let inputDevice of fs.readdirSync(inputDeviceBase)) {
      ...
      let inputName = fs
        .readFileSync(inputDeviceBase + inputDevice + "/device/name", "utf8")
        .trim();
  ...
  static parseEvent(buffer) {
    ...
    // Ignore zero events
    if (rawEvent.type == 0 && rawEvent.code == 0) {
      return null;
    }
    // Ignore repeats
